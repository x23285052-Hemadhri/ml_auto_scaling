...
try:
    s3.download_file(BUCKET_NAME, MODEL_KEY, 'lstm_cpu_model.h5')
    s3.download_file(BUCKET_NAME, SCALER_KEY, 'lstm_scaler.pkl')
    model = load_model('lstm_cpu_model.h5')
...
if len(data_scaled) < SEQUENCE_LENGTH:
    ...
    sys.exit(0)
X_input = data_scaled[-SEQUENCE_LENGTH:].reshape(1, SEQUENCE_LENGTH, len(features))
predicted_cpu = model.predict(X_input)[0][0] * 100
print(f"LSTM predicted CPU: {predicted_cpu:.2f}%")


# === SCALING DECISION ===
cooldown_file = 'last_scale_lstm.txt'
    ...
    MetricData=[
        {
            ...
            'Dimensions': [{'Name': 'Model', 'Value': 'LSTM'}],
...
print("LSTM Predictive Auto-Scaling Controller complete.")
